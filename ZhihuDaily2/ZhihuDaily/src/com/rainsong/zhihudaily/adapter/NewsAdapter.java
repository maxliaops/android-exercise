...
public class NewsAdapter extends BaseAdapter {
    ...
    protected List<RetDataEntity> mDataList;
    private ImageLoader mImageLoader;
    private DisplayImageOptions mOptions;

    public NewsAdapter(Context context, ArrayList<RetDataEntity> list) {
        mContext = context;
        mInflater = (LayoutInflater) context
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        mDataList = list == null ? new ArrayList<RetDataEntity>()
                : new ArrayList<RetDataEntity>(list);
    ...
    public void updateData(ArrayList<RetDataEntity> list) {
        this.mDataList = list == null ? new ArrayList<RetDataEntity>()
                : new ArrayList<RetDataEntity>(list);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        RetDataEntity item = mDataList.get(position);
        ...
        case TYPE_NORMAL:
            ...
            String title = item.getName() + "  " + item.getAge() + "岁  "
                    + item.getJob() + "  \n" + item.getPrice() + "元/时";
             newsTitleView.setText(title);
            mImageLoader.displayImage(item.getPhoto_1(), newsImageView,
                    mOptions);
            return convertView;
        case TYPE_TAG:
            if (convertView == null) {
